...
public class RegionOutlineBuilder {
    ...
    public static Entity buildBlockDisplay(ServerLevel level, BlockPos pos, CompoundTag displayTag) {
        displayTag.putString("id",  EntityTypes.BLOCK_DISPLAY.builtInRegistryHolder().key().identifier().toString());
        return EntityType.loadEntityRecursive(displayTag, level, new EntitySpawnRequest(EntitySpawnReason.COMMAND, true), p_396566_ -> {
