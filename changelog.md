...
* The `lightning` flag now also prevents lightning strikes not hitting entities, prevents creation of fire blocks and de-oxidation of copper blocks. These additional features may be split in separate flags down the road.
* The `access-container` flag now considers all entities implementing the vanilla inventory (like Minecarts with chests).
...
* The command `/wp marker create ...` and `/wp dim <dim> create local ...` now properly checks for permission for the
  parent region
* [Fabric] Fix wither-destruction flag not working correctly due to wrong block pos check
...
# [0.0.2.9-beta4] - 2024-05-13
## Fixed
* [Fabric] Fix sweet berries and glow berries not considered by the place-blocks flag
* [Fabric] Fix issue with the ignite-explosives flag and a crash with explosions
