...
public class ChatComponentBuilder {
    ...
    public static MutableComponent buildGroupLink(IProtectedRegion region, String group, int groupSize) {
        ...
        switch (region.getRegionType()) {
            ...
            case LOCAL: {
                String cmd = buildCommandStr(CommandConstants.LOCAL.toString(), region.getDim().location().toString(), region.getName(), LIST.toString(), CommandConstants.GROUP.toString(), group);
                return buildExecuteCmdComponent(linkText, hoverText, cmd, RUN_COMMAND, LINK_COLOR);
            }
            case DIMENSION: {
    ...
    public static MutableComponent buildRemoveRegionEntry(IProtectedRegion parent, IProtectedRegion region) {
        ...
        switch (parent.getRegionType()) {
            ...
            case DIMENSION: {
                ...
                MutableComponent childCompInfo = new TranslatableComponent("cli.msg.info.dim.region.child.hover");
                MutableComponent childIndicator = buildTextWithHoverAndBracketsMsg(new TextComponent("*"), childCompInfo, GOLD);
                if (parent.hasChild(region)) {
                    regionInfoLinkWithIndicator = new TranslatableComponent("%s%s", buildRegionInfoLink(region), childIndicator);
                } else {
                    regionInfoLinkWithIndicator = new TranslatableComponent("%s", buildRegionInfoLink(region));
                }
                removeLink = buildDimSuggestRegionRemovalLink((IMarkableRegion) region);
                regionRemoveLink = new TranslatableComponent("%s %s", removeLink, buildRegionInfoAndTpLink((IMarkableRegion) region, regionInfoLinkWithIndicator));
                break;
            }
            case LOCAL: {
    ...
    public static MutableComponent buildResponsibleFlagListLink(IProtectedRegion region) {
        ...
        MutableComponent responsibleFlagsNumber = buildTextWithHoverMsg(new TranslatableComponent("%s", flagsInHierarchy.size()),
                new TranslatableComponent("cli.msg.info.region.flag.responsible.number.hover", flagsInHierarchy.size(), region.getName()), LINK_COLOR);
        MutableComponent responsibleFlagListHoverText = new TranslatableComponent("cli.msg.info.region.flag.responsible.link.hover", region.getName());
        switch (region.getRegionType()) {
            case GLOBAL: {
                ...
                return new TranslatableComponent("cli.msg.info.region.flag.responsible.link.text", responsibleFlagsComp);
            }
            case DIMENSION: {
    ...
    public static MutableComponent buildFlagsListLink(IProtectedRegion region) {
        ...
        return new TranslatableComponent("%s | %s", buildResponsibleFlagListLink(region), buildRegionFlagListLink(region));
    }
    ...
    public static MutableComponent buildRegionFlagListLink(IProtectedRegion region) {
        MutableComponent regionFlagNumber = buildTextWithHoverMsg(new TranslatableComponent("%s", region.getFlags().size()),
                new TranslatableComponent("cli.msg.info.region.flag.number.hover", region.getFlags().size(), region.getName()), LINK_COLOR);
    ...
    public static MutableComponent buildDimensionalBlockTpLink(ResourceKey<Level> dim, BlockPos target) {
        ...
        MutableComponent text = new TranslatableComponent("cli.msg.info.region.area.tp.block.link.text", buildBlockPosLinkText(target));
        MutableComponent hover = new TranslatableComponent("cli.msg.info.region.area.tp.block.link.hover");
    ...
    private static MutableComponent buildRegionInfoAndTpLink(IMarkableRegion region, MutableComponent regionInfoLinkWithIndicator) {
        return new TranslatableComponent("%s @ %s", regionInfoLinkWithIndicator, buildRegionTeleportLink(region, null));
    }
