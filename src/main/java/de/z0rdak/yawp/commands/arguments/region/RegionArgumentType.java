...
public class RegionArgumentType implements ArgumentType<String> {
    ...
    public static IProtectedRegion getRegion(CommandContext<CommandSourceStack> context, RegionType regionType) throws CommandSyntaxException {
        switch (regionType) {
            case GLOBAL:
    ...
    public static IMarkableRegion getRegionInPlayerDim(CommandContext<CommandSourceStack> context, String argName) throws CommandSyntaxException {
        ...
        if (region != null) {
            ...
            sendCmdFeedback(context.getSource(), Component.literal("No regions defined in dim '" + dimCache.dimensionKey().location() + "'"));
            throw ERROR_INVALID_VALUE.create(regionName);
        }
    }
