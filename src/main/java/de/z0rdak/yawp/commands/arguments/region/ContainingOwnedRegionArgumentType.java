                ...
                if (StickUtil.isVanillaStick(maybeStick) && StickUtil.isMarker(maybeStick)) {
                    ...
                    if (stickNBT != null) {
                        ...
                        if (overlappingWithPermission.containingRegions.isEmpty()) {
                            sendCmdFeedback(src, Component.translatableWithFallback("cli.arg.area.owned.no-containment", "There is no suitable Local Region as parent for the marked area (no ownership or containment)"));
                            return Suggestions.empty();
                        }
