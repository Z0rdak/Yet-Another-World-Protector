    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        if (context.getSource() instanceof CommandSourceStack src) {
            ...
            if (potentialChildrenNames.isEmpty()) {
                sendCmdFeedback(src, Component.translatableWithFallback("cli.arg.region.add.child.no-valid", "There are no valid child regions for region %s.", buildRegionInfoLink(region)));
