                ...
                if (stickType == StickType.MARKER) {
                    ...
                    if (stickNBT != null) {
                        ...
                        if (!marker.isValidArea()) {
                            sendCmdFeedback(src, new TranslatableComponent("Marked area is not valid").withStyle(RED));
                            return 1;
                        }
                        ...
                        sendCmdFeedback(src, new TranslatableComponent("Invalid marker stick data"));
                        return -2;
                    }
                } else {
                    sendCmdFeedback(src, new TextComponent(RED + "").append(new TranslatableComponent("cli.msg.dim.info.region.create.stick.missing")));
                    return -2;
                }
            } else {
                sendCmdFeedback(src, new TextComponent(RED + "").append(new TranslatableComponent("cli.msg.dim.info.region.create.stick.missing")));
        ...
        try {
            ...
            if (!mainHandItem.equals(ItemStack.EMPTY)
                ...
                if (Objects.requireNonNull(stickType) == StickType.MARKER) {
                    ...
                    sendCmdFeedback(src, new TranslatableComponent("RegionMarker reset!"));
                    return 0;
                } else {
                    sendCmdFeedback(src, new TranslatableComponent(RED + "The item in the main hand is not a RegionMarker!"));
                    return 1;
                }
            } else {
                sendCmdFeedback(src, new TranslatableComponent(RED + "The item in the main hand is not a RegionMarker!"));
