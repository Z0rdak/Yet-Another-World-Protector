                ...
                if (stickType == StickType.MARKER) {
                    ...
                    if (stickNBT != null) {
                        ...
                        if (!marker.isValidArea()) {
                            sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.region.create.stick.area.invalid").formatted(RED));
                            return 1;
                        }
                        ...
                        if (parentRegion != null) {
                            ...
                            if (parentRegion.hasOwner(player.getUuid()) || hasConfigPermission) {
                                if (AbstractMarkableRegion.fullyContains(parentRegion.getArea(), region.getArea())) {
                                    ...
                                    sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.region.create.stick.area.invalid.parent", buildRegionInfoLink(parentRegion, LOCAL)));
                                    return -1;
                                }
                            } else {
                                sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.region.create.local.deny", buildRegionInfoLink(parentRegion, LOCAL)));
                                return 1;
                            }
                        } else {
                            if (dimCache.hasOwner(player) || hasConfigPermission) {
                                ...
                                sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.region.create.dim.deny", buildRegionInfoLink(dimCache.getDimensionalRegion(), RegionType.DIMENSION)));
                                return 2;
                            }
                        }
                    } else {
                        sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.region.create.stick.invalid"));
                        return -2;
                    }
                } else {
                    sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.region.create.stick.missing").formatted(RED));
                    return -2;
                }
            } else {
                sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.region.create.stick.missing").formatted(RED));
        ...
        try {
            ...
            if (!mainHandItem.equals(ItemStack.EMPTY)
                ...
                if (Objects.requireNonNull(stickType) == StickType.MARKER) {
                    ...
                    sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.region.create.stick.reset"));
                    return 0;
                } else {
                    sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.region.create.stick.missing").formatted(RED));
                    return 1;
            ...
            sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.region.create.stick.no-player").formatted(RED));
            return 1;
        }
    }
