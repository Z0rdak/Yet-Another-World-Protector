...
public class RegionCommands {
                ...
                case CUBOID:
                    ...
                    if (parent instanceof IMarkableRegion localParentRegion) {
                        ...
                        if (parentArea.contains(cuboidArea)) {
                            ...
                            MutableText updateAreaFailMsg = new TranslatableText("cli.msg.info.region.spatial.area.update.fail.boundaries",buildRegionInfoLink(parent, LOCAL), buildRegionInfoLink(region, LOCAL));
    ...
    private static int renameRegion(CommandContext<ServerCommandSource> src, IMarkableRegion region, String regionName, DimensionRegionCache dimCache) {
        ...
        if (res == -1) {
            sendCmdFeedback(src.getSource(), new TranslatableText("cli.msg.dim.info.region.create.name.invalid", regionName));
            return res;
        }
        if (res == 1) {
            sendCmdFeedback(src.getSource(), new TranslatableText("cli.msg.dim.info.region.create.name.exists", dimCache.getDimensionalRegion().getName(), buildRegionInfoLink(dimCache.getRegion(regionName), LOCAL)));
            return res;
        }
        // FIXME:
        ...
        switch (affiliation) {
            case "member":
                if (region.hasMember(team.getName())) {
                    ...
                    RegionDataManager.save();
                    sendCmdFeedback(src.getSource(), new TranslatableText("cli.msg.info.region.affiliation.team.removed",
        ...
        switch (affiliation) {
            case "member":
                if (!region.hasMember(team.getName())) {
                    ...
                    RegionDataManager.save();
                    sendCmdFeedback(src.getSource(), new TranslatableText("cli.msg.info.region.affiliation.team.added",
        ...
        switch (affiliation) {
            case "member":
                if (region.hasMember(player.getUuid())) {
                    ...
                    RegionDataManager.save();
                    sendCmdFeedback(src.getSource(), new TranslatableText("cli.msg.info.region.affiliation.player.removed",
        ...
        switch (affiliation) {
            case "member":
                if (!region.hasMember(player.getUuid())) {
                    ...
                    RegionDataManager.save();
                    sendCmdFeedback(src.getSource(), new TranslatableText("cli.msg.info.region.affiliation.player.added",
    ...
    private static int removeChildren(CommandContext<ServerCommandSource> src, DimensionRegionCache dimCache, IMarkableRegion parent, IMarkableRegion child) {
        if (parent.hasChild(child)) {
            ...
            sendCmdFeedback(src.getSource(), new TranslatableText("cli.msg.info.region.children.remove", notLongerChildLink, parentLink));
            sendCmdFeedback(src.getSource(), new TranslatableText("cli.msg.info.region.parent.clear", notLongerChildLink, dimensionalLink));
            return 0;
        }
    ...
    private static int addChildren(CommandContext<ServerCommandSource> src, IMarkableRegion parent, IMarkableRegion child) {
        if (!parent.hasChild(child) && child.getParent() != null && child.getParent() instanceof DimensionalRegion) {
            ...
            MutableText undoLink = buildRegionActionUndoLink(src.getInput(), ADD, REMOVE);
            sendCmdFeedback(src.getSource(), new TranslatableText("cli.msg.info.region.children.add", childLink, parentLink).append(" ").append(undoLink));
            return 0;
        }
    ...
    private static int removeFlag(CommandContext<ServerCommandSource> src, IMarkableRegion region, RegionFlag flag) {
        if (region.containsFlag(flag)) {
            ...
            RegionDataManager.save();
            sendCmdFeedback(src.getSource(), new TranslatableText("cli.msg.flags.removed",
    ...
    private static int setAlertState(CommandContext<ServerCommandSource> src, IMarkableRegion region, boolean showAlert) {
        ...
        if (oldState == region.isMuted()) {
            ...
            sendCmdFeedback(src.getSource(), new TranslatableText("cli.msg.info.region.state.alert.set.value",
    ...
    private static int setEnableState(CommandContext<ServerCommandSource> src, IMarkableRegion region, boolean enable) {
        ...
        if (oldState != region.isActive()) {
            ...
            sendCmdFeedback(src.getSource(), new TranslatableText("cli.msg.info.region.state.enable.set.value",
    ...
    private static int setPriority(CommandContext<ServerCommandSource> src, IMarkableRegion region, int priority, int factor) {
        ...
        if (Integer.MAX_VALUE - newValue > 0) {
            ...
            sendCmdFeedback(src.getSource(), new TranslatableText("cli.msg.warn.region.state.priority.set.invalid", buildRegionInfoLink(region, LOCAL), newValue));
            return -1;
        }
    }
    ...
    private static int setPriority(CommandContext<ServerCommandSource> src, IMarkableRegion region, int priority) {
        ...
        if (existRegionWithSamePriority) {
            ...
            if (oldPriority != priority) {
                ...
                sendCmdFeedback(src.getSource(), new TranslatableText("cli.msg.info.region.state.priority.set.success",
                        buildRegionInfoLink(region, LOCAL), oldPriority, region.getPriority()).append(" ").append(undoLink));
                return 0;
            } else {
                sendCmdFeedback(src.getSource(), new TranslatableText("cli.msg.info.region.state.priority.set.fail.no-change", buildRegionInfoLink(region, LOCAL)));
                return 1;
            }
        }
    }
    ...
    private static int updateRegion(CommandContext<ServerCommandSource> src, IMarkableRegion region) {
        try {
            PlayerEntity player = src.getSource().getPlayer();
    ...
    private static int setTeleportPos(CommandContext<ServerCommandSource> src, IMarkableRegion region, BlockPos target) {
        if (!region.getTpTarget().equals(target)) {
            ...
            sendCmdFeedback(src.getSource(), new TranslatableText("cli.msg.info.region.spatial.location.teleport.set", buildRegionInfoLink(region, LOCAL), newTpTargetLink));
            return 0;
        }
        return 1;
    }
