...
public class DimensionCommands {
    ...
    private static int resetLocalRegions(CommandContext<CommandSourceStack> ctx, DimensionRegionCache dimCache) {
        ...
        RegionDataManager.save();
        sendCmdFeedback(ctx.getSource(), Component.translatableWithFallback("cli.msg.info.dim.reset.all.confirm", "Successfully reset all local regions in %s", buildRegionInfoLink(dimCache.getDimensionalRegion())));
        return 0;
    }
    ...
    private static int resetDimRegion(CommandContext<CommandSourceStack> ctx, DimensionRegionCache dimCache) {
        ...
        RegionDataManager.save();
        sendCmdFeedback(ctx.getSource(), Component.translatableWithFallback("cli.msg.info.dim.reset.confirm", "Successfully reset dimensional region %s", buildRegionInfoLink(dimCache.getDimensionalRegion())));
        return 0;
    }
    ...
    private static int createRegion(CommandContext<CommandSourceStack> ctx, String regionName, DimensionRegionCache dimCache, IMarkableRegion region, IProtectedRegion parent) {
        ...
        if (parent.getRegionType() != RegionType.DIMENSION && parent.getRegionType() != RegionType.LOCAL) {
            sendCmdFeedback(ctx.getSource(), Component.translatableWithFallback("cli.msg.dim.info.region.create.error", "Error creating region %s", buildRegionInfoLink(parent)));
            return -1;
        }
        ...
        RegionDataManager.save();
        sendCmdFeedback(ctx.getSource(), Component.translatableWithFallback("cli.msg.dim.info.region.create.success", "Successfully created region %s (with parent %s)", buildRegionInfoLink(region), buildRegionInfoLink(parent)));
        return 0;
    }
    ...
    private static int deleteRegion(CommandContext<CommandSourceStack> ctx, DimensionRegionCache dim, IMarkableRegion region) {
        ...
        if (NeoForge.EVENT_BUS.post(new RegionEvent.RemoveRegionEvent(region, player)).isCanceled()) {
            return 0;
        }
        if (dim.contains(region.getName())) {
            if (!region.getChildren().isEmpty()) {
                sendCmdFeedback(ctx.getSource(), Component.translatableWithFallback("cli.msg.info.dim.region.remove.fail.hasChildren", "Region %s can't be deleted because it has child regions.", buildRegionInfoLink(region)));
                return -1;
            }
            ...
            if (parentType == RegionType.DIMENSION) {
                ...
                RegionDataManager.save();
                sendCmdFeedback(ctx.getSource(), Component.translatableWithFallback("cli.msg.info.dim.region.remove.confirm", "Removed region '%s' from %s", region.getName(), buildRegionInfoLink(dim.getDimensionalRegion())));
                return 0;
            }
            if (parentType == RegionType.LOCAL) {
                sendCmdFeedback(ctx.getSource(), Component.translatableWithFallback("cli.msg.info.dim.region.remove.fail.hasParent", "Region %s can't be deleted because it has a Local Regions as parent.", buildRegionInfoLink(region)));
                return 1;
            }
        }
        return 1;
    ...
    private static int deleteRegions(CommandContext<CommandSourceStack> ctx, DimensionRegionCache dimCache) {
        ...
        RegionDataManager.save();
        sendCmdFeedback(ctx.getSource(), Component.translatableWithFallback("cli.msg.info.dim.region.remove.all.confirm", "Removed %s regions from dimension %s", amount, buildRegionInfoLink(dimCache.getDimensionalRegion())));
        return 0;
    }
