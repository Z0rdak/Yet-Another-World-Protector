...
public class DimensionCommands {
    ...
    private static int createCuboidRegion(ServerCommandSource src, String regionName, DimensionRegionCache dimCache, BlockPos pos1, BlockPos pos2, ServerPlayerEntity owner) {
        ...
        if (res == 1) {
            ...
            sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.region.create.name.exists", dimCache.getDimensionalRegion().getName(), buildRegionInfoLink(dimCache.getRegion(regionName), LOCAL)));
            return res;
        }
    ...
    private static int createSphereRegion(ServerCommandSource src, String regionName, DimensionRegionCache dimCache, BlockPos center, BlockPos outerPos, ServerPlayerEntity owner) {
        ...
        if (dimCache.contains(regionName)) {
            sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.region.create.name.exists", dimCache.dimensionKey(), regionName));
            return 1;
        }
    ...
    private static int attemptDeleteRegion(ServerCommandSource src, DimensionRegionCache dim, IMarkableRegion region) {
        if (dim.contains(region.getName())) {
            sendCmdFeedback(src, new TranslatableText("cli.msg.info.dim.region.remove.attempt", buildRegionInfoLink(region, LOCAL), buildRegionInfoLink(dim.getDimensionalRegion(), RegionType.DIMENSION)));
            return 0;
        }
        return 1;
    }
    ...
    private static int deleteRegion(ServerCommandSource src, DimensionRegionCache dim, IMarkableRegion region) {
        if (dim.contains(region.getName())) {
            if (!region.getChildren().isEmpty()) {
                ...
                sendCmdFeedback(src, new TranslatableText("cli.msg.info.dim.region.remove.fail.hasChildren", buildRegionInfoLink(region, LOCAL)));
                return -1;
            }
            ...
            RegionDataManager.save();
            sendCmdFeedback(src, new TranslatableText("cli.msg.info.dim.region.remove.confirm", region.getName(), buildRegionInfoLink(dim.getDimensionalRegion(), RegionType.DIMENSION)));
            return 0;
        }
        return 1;
    }
    ...
    private static int removeFlag(ServerCommandSource src, DimensionRegionCache dimCache, String flag) {
        if (dimCache != null) {
            ...
            sendCmdFeedback(src, new TranslatableText("cli.msg.flags.removed", flag, buildRegionInfoLink(dimCache.getDimensionalRegion(), RegionType.DIMENSION)));
            return 0;
        }
        return 1;
    }
    ...
    private static int addFlag(ServerCommandSource src, DimensionRegionCache dimCache, String flag) {
        ...
        if (dimCache != null) {
            ...
            sendCmdFeedback(src, new TranslatableText("cli.msg.flags.added", flag, buildRegionInfoLink(dimCache.getDimensionalRegion(), RegionType.DIMENSION)));
            return 0;
        }
        return 1;
    }
        ...
        if (dimCache != null) {
            if (affiliationType.equals(MEMBER.toString())) {
                if (dimCache.getDimensionalRegion().hasMember(player.getUuid())) {
                    ...
                    sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.player.removed", affiliationType, playerInfo, buildRegionInfoLink(dimCache.getDimensionalRegion(), RegionType.DIMENSION)));
                    RegionDataManager.save();
                    return 0;
                }
            }
        ...
        if (dimCache != null) {
            if (affiliationType.equals(MEMBER.toString())) {
                if (dimCache.getDimensionalRegion().hasMember(team.getName())) {
                    ...
                    sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.player.removed", affiliationType, buildTeamHoverComponent(team), buildRegionInfoLink(dimCache.getDimensionalRegion(), RegionType.DIMENSION)));
                    RegionDataManager.save();
                    return 0;
                }
            }
    ...
    private static int addPlayer(ServerCommandSource src, ServerPlayerEntity player, DimensionRegionCache dimCache, String affiliationType) {
        if (dimCache != null) {
            ...
            sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.player.added", buildPlayerHoverComponent(player), dim.getValue().toString(), affiliationType));
            return 0;
        }
        return 1;
    }
    ...
    private static int addTeam(ServerCommandSource src, Team team, DimensionRegionCache dimCache, String affiliationType) {
        if (dimCache != null) {
            ...
            sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.team.added", buildTeamHoverComponent(team), dim.getValue().toString(), affiliationType));
            return 0;
        }
        return 1;
    }
    ...
    private static int setActiveState(ServerCommandSource src, DimensionRegionCache dimCache, boolean activate) {
        if (dimCache != null) {
            ...
            sendCmdFeedback(src, new TranslatableText(langKey, buildRegionInfoLink(dimCache.getDimensionalRegion(), RegionType.DIMENSION)));
            return 0;
        }
        return 1;
    }
    ...
    private static int promptDimensionRegionList(ServerCommandSource src, DimensionRegionCache dimCache, int pageNo) {
        if (dimCache != null) {
            ...
            if (regionsForDim.isEmpty()) {
                sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.regions.empty", buildRegionInfoLink(dimCache.getDimensionalRegion(), RegionType.DIMENSION)));
                return -1;
            }
            List<MutableText> regionPagination = buildPaginationComponents(
                    ...
                    pageNo,
                    new LiteralText(" - ").append(buildDimCreateRegionLink(dimRegion)));
    ...
    private static int promptDimensionFlagList(ServerCommandSource src, DimensionRegionCache dimCache, int pageNo) {
        ...
        if (flags.isEmpty()) {
            sendCmdFeedback(src, new TranslatableText("cli.msg.dim.info.flags.empty", buildRegionInfoLink(dimCache.getDimensionalRegion(), RegionType.DIMENSION)));
            return 1;
        }
        List<MutableText> flagPagination = buildPaginationComponents(
                ...
                pageNo,
                new LiteralText(" - ").append(buildDimAddFlagLink(dimCache.getDimensionalRegion())));
    ...
    private static int promptDimensionAffiliationList(ServerCommandSource src, DimensionRegionCache dimCache, String affiliation, AffiliationType affiliationType, int pageNo) {
        ...
        if (affiliateNames.isEmpty()) {
            sendCmdFeedback(src, new TranslatableText("cli.msg.info.region.affiliation." + affiliationType.name + ".empty", affiliation, buildRegionInfoLink(dimCache.getDimensionalRegion(), RegionType.DIMENSION)));
            return 1;
        }
        List<MutableText> affiliatePagination = buildPaginationComponents(
                ...
                pageNo,
                new LiteralText(" - ").append(buildAddAffiliateLink(dimRegion, affiliation, affiliationType, RegionType.DIMENSION)));
