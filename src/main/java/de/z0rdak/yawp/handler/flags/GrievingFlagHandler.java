        ...
        if (isServerSide(event)) {
            ...
            if (dimCache != null) {
                ...
                if (isPlayer(event.getSource().getEntity())) {
                    FlagCheckEvent.PlayerFlagEvent playerFlagCheckEvent = checkPlayerEvent((PlayerEntity) event.getSource().getEntity(), lootEntity.blockPosition(), RegionFlag.DROP_LOOT_PLAYER, dimCache.getDimensionalRegion());
                    handleAndSendMsg(event, playerFlagCheckEvent);
                }
            }
        }
