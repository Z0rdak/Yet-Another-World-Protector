...
public class WorldFlagHandler {
    ...
    public static void onUsePortal(EntityTravelToDimensionEvent event) {
        if (isServerSide(event.getEntity())) {
            ...
            if (NeoForge.EVENT_BUS.post(checkGeneralEvent).isCanceled()) {
                return;
            }
            ...
            if (entity instanceof Player) {
                ...
                if (NeoForge.EVENT_BUS.post(checkPlayerEvent).isCanceled()) {
                    return;
                }
                ...
                if (nonPlayerCheckEvent != null) {
                    if (NeoForge.EVENT_BUS.post(nonPlayerCheckEvent).isCanceled()) {
                        return;
                    }
